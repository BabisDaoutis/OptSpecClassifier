...
models_dir = os.path.join(PARENT_DIR, 'OptSpecClassifier')

clf = joblib.load(os.path.join(models_dir, 'Models/Auto_OptSepcClassifier_SVM.sav'))
scaler_sv = joblib.load(os.path.join(models_dir, 'Models/Auto_OptSepcClassifier_scaler.sav'))

scaler_AGN_Type = joblib.load(os.path.join(models_dir, 'Models/Auto_OptSepcClassifier_scaler_AGN_type.sav'))
clf_AGN_Type = joblib.load(os.path.join(models_dir, 'Models/Auto_OptSepcClassifier_SVM_AGN_type.sav'))
...
def calc_eqw_mc(wavelength, flux, sigma, line_center, line_width, cont_width, mc_iter):
    ...
    sigma : array_like
        ...
        Note: Current logic uses measurement uncertainty of the continuum for MC noise generation.
    line_center : float
...
def rf_classify_mc_spectra(wavelength, flux, sigma, plot=False, kinematics=False, n_mc=100, clf=clf, scaler_sv=scaler_sv):
    """`
    ...
    clf : sklearn.ensemble.SVC
        The pre-trained Support Vector Machine classifier object.
    ...
    kinematics : bool, if this flag is true, the function will calculate kinematic properties.
    plot : bool
    ...
    if kinematics and majority_class == "AGN":
        FWQM_Ha = fwqm(flux, wavelength, 'ha')
    ...
    if kinematics == True:
        broad_line_flg = check_fwqm_ratio(flux, wavelength)
        if broad_line_flg == 'Potential Outflow':
            broad_line_flag = 1
        else:
            broad_line_flag = 0
    else:
        broad_line_flag = np.nan
    ...
    if plot:
        ...
        return majority_class, mean_probabilities, std_probabilities, AGN_type, broad_line_flag
    

def rf_classify_mc_EWs(EWs, EWs_unc, plot=False, n_mc=100, clf=clf, scaler_sv=scaler_sv):
    """
    Performs Monte Carlo spectral classification using a Support Vector Machine classifier based on 
    emission line Equivalent Widths (EWs).
...
def continuum(wavelength, flux, line_center, line_width, cont_width, lm):
    """
    Calculates the continuum level at a specific wavelength using linear interpolation
    between sidebands.

    This function determines the continuum by taking the median flux and wavelength
    in two windows (sidebands) located to the left and right of the spectral line.
    It then fits a straight line between these two points and evaluates it at the
    target wavelength `lm`.

    Parameters
    ----------
    wavelength : pandas.Series or array-like
        The wavelength array.
    flux : pandas.Series or array-like
        The spectral flux array.
    line_center : float
        The central wavelength of the emission/absorption line (used to anchor the windows).
    line_width : float
        The width of the exclusion zone around the line center (total width excluded).
        The left window starts at `line_center - line_width/2`.
    cont_width : float
        The width of the continuum windows (sidebands) themselves.
    lm : float
        The specific wavelength at which to evaluate the continuum level (usually 
        the line center or peak location).

    Returns
    -------
    float
        The estimated continuum flux value at wavelength `lm`.

    Notes
    -----
    The function uses a linear equation $y = mx + c$ determined by two points:
    1. Left Point: (Median Wavelength, Median Flux) of the left sideband.
    2. Right Point: (Median Wavelength, Median Flux) of the right sideband.
    
    Equation used:
    $$ Continuum = m \times lm + c $$
    where $m$ is the slope between the two sideband medians.
    """
    cont_range_lf = (wavelength < line_center - line_width/2) & (wavelength > line_center - line_width/2 - cont_width)
    cont_range_rt = (wavelength > line_center + line_width/2) & (wavelength < line_center + line_width/2 + cont_width)
...
def fwqm(flux, wavelength, line):
    """
    Calculates the Full Width at Quarter Maximum (FWQM) for a specified spectral line.

    This function isolates a specific spectral line ('ha', 'hb', or 'oiii'), determines
    the continuum and peak flux, and calculates the width of the line at 25% of the
    peak height above the continuum level. It uses interpolation to determine
    ...
    wavelength : pandas.Series or array-like
        The wavelength values corresponding to the flux. Assumed to be in Angstroms
        given the hardcoded line centers (e.g., H-alpha at 6564).
    line : {'ha', 'hb', 'oiii'}
        The specific spectral line to analyze.
        - 'ha': H-alpha (Target: 6564 Å)
        - 'hb': H-beta (Target: 4864 Å)
        - 'oiii': [O III] (Target: 5007 Å)
    ...
    Raises
    ------
    bdb.BdbQuit (via breakpoint)
        If an invalid `line` string is provided, the code triggers a breakpoint.

    Notes
    -----
    The quarter maximum level is calculated as:
    Level_FWQM = (Peak + 3*Continuum/4

    This is equivalent to: Continuum + 0.25*(Peak - Continuum).

    Dependencies
    ------------
    This function relies on the following external/global functions:
    - `continuum()`: Estimates the continuum level.
    - `peak_flux()`: Finds the peak flux within a window.
    - `find_closest_point()`: Finds the index of a value in an array.
    - `scipy.interpolate.PchipInterpolator`: For flux interpolation.
    """
    if line == 'ha':
        line_center = 6564
        line_width = 80
...
def check_fwqm_ratio(flux, wavelength):
    """
    Calculates the ratio of Full Width at Quarter Maximum (FWQM) between H-alpha and [OIII] lines.
    This function computes the FWQM for both the H-alpha and [OIII] spectral lines
    and returns their ratio to flag potential line broadening from outflows.
    ...
    flux : pandas.Series or array-like
        The spectral flux values.
    wavelength : pandas.Series or array-like
        The wavelength values corresponding to the flux.
    Returns
    -------
    float
        The ratio of FWQM between H-alpha and [OIII] lines.
    """
    fwqm_oiii = fwqm(flux, wavelength, 'oiii')
    fwqm_ha = fwqm(flux, wavelength, 'ha')
    fwqm_ratio = fwqm_ha/fwqm_oiii
    if fwqm_ratio < 1.75:
        flag = 'Normal'
    else:
        flag = 'Potential Outflow'
    return flag
